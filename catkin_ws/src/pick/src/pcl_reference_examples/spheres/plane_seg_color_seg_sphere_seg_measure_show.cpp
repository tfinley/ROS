...
main (int argc, char** argv)
{
  ...
  //color filter
  int rMin = 100; //atoi(argv[2]);
  int rMax = 255; //atoi(argv[3]);
  int gMin = 0; //atoi(argv[4]);
  int gMax = 120; //atoi(argv[5]);
  int bMin = 0; //atoi(argv[6]); 
  int bMax = 120; //atoi(argv[7]);
  ...
  outrem.setInputCloud(cloud_filtered3);
  float radius = 0.008;
  outrem.setRadiusSearch (radius);
  outrem.setMinNeighborsInRadius (2);
  outrem.filter(*cloud_filtered3);

  writer.write ("planeRemoved.pcd", *cloud_filtered3, false);
  ...
  tree->setInputCloud (cloud_filtered3);
  ...
  for (std::vector<pcl::PointIndices>::const_iterator it = cluster_indices.begin (); it != cluster_indices.end (); ++it)
  {
    	...
    	for (std::vector<int>::const_iterator pit = it->indices.begin (); pit != it->indices.end (); pit++)
      	  cloud_cluster->points.push_back (cloud_filtered3->points[*pit]); //*
