...
boost::shared_ptr<pcl::visualization::PCLVisualizer> customColourVis (pcl::PointCloud<pcl::PointXYZRGB>::ConstPtr cloudIn)
{
  ...
  std::cout << "Cloud before filtering: " << cloudIn->points.size() << std::endl;  
  ...
  //viewer->addCube (translation,rotation, fabs(max_pt.x-min_pt.x), fabs(max_pt.y-min_pt.y), fabs(max_pt.z-min_pt.z), "cube");
