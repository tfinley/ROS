...
{ 
  ...
  */
  ...
  cout << "creating new points in cloud" << endl;
  //create new cloud to define points for line (arrow)
  ...
  while (!viewer->wasStopped ())
  {
	...
	/* - dont need this anymore (it helped with testing purposes to make sure the point picked was correct)
	...
	*/
	...
	Tm << 1, 0, 0, picked_x,
				...
				0, 0, 1, picked_z,
				0, 0, 0, 1;
	pcl::transformPointCloud(*cloud_source, *cloud_source, Tm);
	segmented = false;
	translated = true;
	...
	pcl::transformPointCloud (*cloud_line, *cloud_line, Tm);
  	...
  	p2.y = 0.0;
  	p2.z = 0.0273;
	//p2 = pcl::transformPoint(p2,Tm);
 	viewer->addSphere(cloud_line->points[0], 0.005, 1.0, 0.0, 0.0, "Center", 0);

	//add line 
	viewer->addLine<pcl::PointXYZ> (cloud_line->points[0],cloud_line->points[1], 1.0,0.0,0.0, "line");
