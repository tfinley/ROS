...
{
  ros::init(argc, argv, "publish_part_image");
  ros::NodeHandle nh;

  ros::Publisher pub = nh.advertise<sensor_msgs::Image>("/study/rgb/part_image", 1);
  ...
  while (nh.ok()) 
  {
    ...
    int part_number;
    nh.getParam("/study/part_number", part_number);

    //Setup image file path
    std::string package_path = ros::package::getPath("pick");  
    std::string file_path ;
    std::stringstream sstm;
    sstm << package_path << "/part_images/" << part_number << ".jpeg";
    file_path = sstm.str();
