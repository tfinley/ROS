...
Description: 	The purpose of this node is to take in a segmented point cloud around two spheres
				...
				center of the two spheres. This meathod will also use an average approach.
...
align2_cb (const sensor_msgs::PointCloud2ConstPtr& cloud2)
{
  ...
  {  
	...
	dist = sqrt(pow((x1-x2),2) + pow((y_1-y_2),2) + pow((z1-z2),2));
    ...
    if (measured == sample_size){
		...
		float average = 0;
		for( int k = 0; k < sample_size; k++) average += dists[k];
		average/=sample_size;
		std::cout << "Average: " << average << std::endl;
		//reset distances array		
		//for (int a=0; a<sample_size; a++) dists[a]=0;
		//reset picked bool to stop measuring loops
